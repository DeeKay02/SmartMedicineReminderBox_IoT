...
def set_time(alarm_hr,alarm_min): 
    ...
    lid_response = mybolt.digitalRead('0') #Medical Box sensor fixed at pin0; Taking response from pin0 
    lid_data = json.loads(lid_response)  #Coversion of any string in to json(object notation)
    ...
    try:
        ...
        lid = int(lid_data['value']) 
        button = int(button_data['value']) 
        if (button ==1 and lid==0): #Checking if the buttton is press or not when the lid is open.
            button_state=1 
